...
if __name__ == '__main__':
    ...
    # Output: {0: [], 1: [0], 2: [0], 3: [1, 2], 4: [1, 2, 3]} <-- This is just a list with extra steps, I'm stupid
    connections = {}
    for board_key in board_keys:
        board = checked_boards[board_key]['board']
        connections[board_index_hash[board_key]] = [board_index_hash[sub_board.string_rep] for sub_board in board.get_sub_boards()]

    import numpy as np
    adjacency_matrix = np.zeros((len(board_keys), len(board_keys)))
    for key in connections.keys():
        for connection in connections[key]:
            if checked_boards[board_key]['win_flag'] == True:
                print(f'{key}: True')
                adjacency_matrix[key, connection] = 1
            elif checked_boards[board_key]['win_flag'] == False:
                print(f'{key}: False')
                adjacency_matrix[key, connection] = 2
